(function () {
	let deck = {
		...
		checkForm: function (fields) {
			...
			let editCard = {
				title: function () {
					...
					let editTitleInput = document.getElementById(fields.idx).querySelector('[name="title"]');
		...
		saveEditedCardToDb: function (id, title, text, idx) {
			...
			}).then(() => view.showCard(idx, 'editDel')).catch(err => console.log(err));
		},
		saveCardToDb: function () {
			...
			}).then(() => view.showPage(1, this.modes.view)).catch(err => console.log(err));
		},
		fetchCards: function () {
			...
			//	    .catch((err) => console.log(err));
			fetch(curl).then(result => result.json()).then(data => this.saveCards(data)).then(() => view.showPage(view.currentPage, view.currentMode)).catch(err => console.log(err));
		}
	};
