...
(function () {
	var deck = {
		...
		addCard: function addCard(title, text, author) {
			...
			xhrService.makeXHR('POST', { 'title': title, 'text': text, 'author': author, 'image_url': this.appconst.image_url });
		},
		editCard: function editCard(id, title, text, idx) {
			...
			xhrService.makeXHR('PATCH', { 'title': title, 'text': text }, id, idx);
		},
		...
		//  keeping this around for fetech support down the road
	...
	var xhrService = {
		...
		makeXHR: function makeXHR(method, cardData, id, idx) {
			...
			switch (method) {
				...
				case this.methods.post:
					...
					xhr.onload = function () {
						view.showPage(1, deck.modes.view);
					};
					xhr.send(data);
					break;
				case this.methods.patch:
					xhr.open(method, deck.appconst.base_url + '/' + id, true);
					xhr.responseType = 'json';
					xhr.setRequestHeader('Content-Type', 'application/json');
					xhr.error = function () {
						console.log(xhr.status);
	...
	var handlers = {
		...
		deleteCard: function deleteCard(idx) {
			...
			deck.deleteCard(idx);
			xhrService.makeXHR('DELETE', null, cardId, null);
		},
		...
		removeCard: function removeCard(idx) {
			...
			async function removeElm(toAdd) {
				...
				if (!!toAdd) {
					...
					//				currentPage =
					//					Math.ceil((parseInt(elmMain.lastChild.getAttribute('id'))+1)/view.cardsPerPage);
